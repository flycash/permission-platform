...
- 子账号biz=3从哪里来 —— 业务管理员（biz=2操作者）是否也要允许其操作businessConfig表 
- 业务管理员，如何从系统管理员，获取操作 biz=3的行
- 业务管理员（biz=2操作者）是否要与被创建出来的业务管理员（biz=3）之间建立包含关系 —— biz=2是否默认对biz=3就有管理员权限
  - 包含关系改造，允许跨业务角色包含？？？
  - ![./img.png](./img.png)
- 被创建出来业务管理员（biz=3）是否可以继续创建子账号？（子账号权限传递问题）

- 将子账号问题泛化，即从biz=1创建biz=2时，是不是就可以理解为子账号，那么创建“子账号”的权限在biz=1中就应该提前建立好，还有“子账号权限”授予权限，也应该建立好。这样
  biz=2到biz=3，就简单了。整个模型也统一一些。