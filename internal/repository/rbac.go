...
type RBACRepository interface {
	...
	FindPermissions(ctx context.Context, bizID int64, resourceKey, action string) ([]domain.Permission, error)
	// 角色相关方法
...
func (r *rbacRepository) FindPermissions(ctx context.Context, bizID int64, resourceKey, action string) ([]domain.Permission, error) {
	permissions, err := r.permissionDAO.FindPermissions(ctx, bizID, resourceKey, action)
	if err != nil {
		return nil, err
	}
